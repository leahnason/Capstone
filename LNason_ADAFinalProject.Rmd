...
model.PInternet <- glm(TotalRate ~ HouseholdInternetPercent25 + BlackAfricanAmericanPercent + HouseholdMedianIncome, family=poisson, data=Data)
summary(model.PInternet) 

# Test for overdispersion by running a negative binomial model
model.1Internet <-  glm.nb(TotalRate ~ HouseholdInternetPercent25 + BlackAfricanAmericanPercent + HouseholdMedianIncome, control=glm.control(maxit=50), data = Data)
...
model.0 <- glm(TotalRate ~ HouseholdInternetPercent + BlackAfricanAmericanPercent + HouseholdMedianIncome, family = poisson, data = Data)

# To test for effect modification, include cross-product interaction term in model (adjust) and compare to model without
model.0ef <- glm(TotalRate ~ HouseholdInternetPercent + BlackAfricanAmericanPercent + HouseholdMedianIncome + HealthInsurancePercent*HouseholdInternetPercent, family = poisson, data = Data)
summary(model.0)
